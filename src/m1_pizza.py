...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    points_seq = generate_points_on_circle(circle, number_of_points)
    for k in range(len(points_seq)):
        circle_edge = rg.Circle(points_seq[k], 10)
        circle_edge.fill_color = color
        circle_edge.attach_to(window)
    window.render()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  a lot of slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # -------------------------------------------------------------------------
    circle.attach_to(window)
    line_end = generate_points_on_circle(circle, number_of_slices)
    for k in range(len(line_end)):
        line = rg.Line(circle.center, line_end[k])
...
def run_test_polygon():
    ...
    polygon(window, circle, 4, 'black', 2)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # -------------------------------------------------------------------------
    circle.attach_to(window)
    line_start = generate_points_on_circle(circle, (number_of_segments))
    for k in range(len(line_start)):
        line = rg.Line(line_start[k - 1], line_start[k])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 70, 20, 'lime green', 2)
    window.close_on_mouse_click()
